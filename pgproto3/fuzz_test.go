...
func FuzzFunctionCall(f *testing.F) {
	...
	f.Add([]byte{})
	// Truncated messages. Before the bounds checks were added each of these
	// read past the end of src: an argument format code, the result format
	// code, an argument length, and an argument value respectively.
...
func FuzzSASLInitialResponse(f *testing.F) {
	...
	f.Add([]byte{})
	// Truncated: fewer than 4 bytes remain for the data size after the auth
	// mechanism, which sliced past the end of src before the bounds check.
...
func FuzzCopyFail(f *testing.F) {
	f.Add([]byte{'o', 'o', 'p', 's', 0})
	// An empty message made idx == len(src)-1 == -1 compare equal, so the
	// terminator check passed and src[:idx] sliced with a negative index.
	f.Add([]byte{})
	f.Fuzz(func(t *testing.T, data []byte) {
